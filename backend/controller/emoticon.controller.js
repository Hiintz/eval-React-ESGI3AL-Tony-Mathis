...
exports.remove = async (req, res) => {
    let emoticon = await Emoticon.findOne({ where: { postId: req.params.id, userId: req.token.id } });
    if (!emoticon) {
        return res.status(400).json({ error: "Vous ne pouvez pas retirer un emoticon que vous n'avez pas ajouté" });
    }
    let result = await Emoticon.destroy({ where: { id: req.params.id, userId: req.token.id } });
